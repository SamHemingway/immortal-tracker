export const RACES = [
  {
    raceID: "brt",
    name: "Bretonnia",
    type: "order",
    colour: "hsl(43deg 55% 47%)",
    lords: [
      {
        lordID: "alberic",
        name: "Alberic de Bordeleaux",
        factionName: "Bordeleaux",
      },
      {
        lordID: "fay",
        name: "Fay Enchantress",
        factionName: "Carcassonne",
      },
      {
        lordID: "louen",
        name: "Louen Leoncoeur",
        factionName: "Couronne",
      },
      {
        lordID: "repanse",
        name: "Repanse de Lyonesse",
        factionName: "Lyonesse",
      },
    ],
  },
  {
    raceID: "bst",
    name: "Beastmen",
    type: "chaos",
    colour: "hsl(43deg 55% 47%)",
    lords: [
      {
        lordID: "khazrak",
        name: "Khazrak the One-Eye",
        factionName: "Warherd of the One-Eye",
      },
      {
        lordID: "malagor",
        name: "Malagor the Dark Omen",
        factionName: "Harbinger of Disaster",
      },
      {
        lordID: "morghur",
        name: "Morghur the Shadowgrave",
        factionName: "Warherd of the Shadowgrave",
      },
      {
        lordID: "taurox",
        name: "Taurox the Brass Bull",
        factionName: "Slaughterhorn Tribe",
      },
    ],
  },
  {
    raceID: "chd",
    name: "Chaos Dwarfs",
    type: "chaos",
    colour: "hsl(15deg 99% 44%)",
    lords: [
      {
        lordID: "astragoth",
        name: "Astragoth Ironhand",
        factionName: "Disciples of Hashut",
      },
      {
        lordID: "drazhoath",
        name: "Drazhoath the Ashen",
        factionName: "Legion of Azgorh",
      },
      {
        lordID: "zhatan",
        name: "Zhatan the Black",
        factionName: "The Warhost of Zharr",
      },
    ],
  },
  {
    raceID: "cst",
    name: "Vampire Coast",
    type: "undead",
    colour: "hsl(182deg 64% 50%)",
    lords: [
      {
        lordID: "noctilus",
        name: "Count Noctilus",
        factionName: "The Dreadfleet",
      },
      {
        lordID: "luthor",
        name: "Count Luthor Harkon",
        factionName: "The Awakened",
      },
      {
        lordID: "aranessa",
        name: "Aranessa Saltspite",
        factionName: "Pirates of Sartosa",
      },
      {
        lordID: "cylostra",
        name: "Cylostra Direfin",
        factionName: "The Drowned",
      },
    ],
  },
  {
    raceID: "cth",
    name: "Grand Cathay",
    type: "order",
    colour: "hsl(352deg 100% 50%)",
    lords: [
      {
        lordID: "miao",
        name: "Miao Ying",
        factionName: "The Northern Provinces",
      },
      {
        lordID: "zhao",
        name: "Zhao Ming",
        factionName: "The Western Provinces",
      },
    ],
  },
  {
    raceID: "dae",
    name: "Daemons of Chaos",
    type: "chaos",
    colour: "hsl(352deg 100% 50%)",
    lords: [
      {
        lordID: "daemon",
        name: "Daemon Prince",
        factionName: "Legion of Chaos",
      },
    ],
  },
  {
    raceID: "def",
    name: "Dark Elves",
    type: "destruction",
    colour: "hsl(248deg 65% 66%)",
    lords: [
      {
        lordID: "lokhir",
        name: "Lokhir Fellheart",
        factionName: "The Blessed Dread",
      },
      {
        lordID: "malekith",
        name: "Malekith",
        factionName: "Naggarond",
      },
      {
        lordID: "morathi",
        name: "Morathi",
        factionName: "Cult of Pleasure",
      },
      {
        lordID: "hellebron",
        name: "Crone Hellebron",
        factionName: "Har Ganeth",
      },
      {
        lordID: "malus",
        name: "Malus Darkblade",
        factionName: "Hag Graef",
      },
      {
        lordID: "rakarth",
        name: "Rakarth",
        factionName: "The Thousand Maws",
      },
    ],
  },

  {
    raceID: "dwf",
    name: "Dwarfs",
    type: "order",
    colour: "hsl(182deg 64% 50%)",
    lords: [
      {
        lordID: "grombrindal",
        name: "Grombrindal",
        factionName: "The Ancestral Throng",
      },
      {
        lordID: "thorgrim",
        name: "Thorgrim Grudgebearer",
        factionName: "Karaz-a-Karak",
      },
      {
        lordID: "ungrim",
        name: "Ungrim Ironfist",
        factionName: "Karak Kadrin",
      },
      {
        lordID: "belegar",
        name: "Belegar Ironhammer",
        factionName: "Clan Angrund",
      },
      {
        lordID: "thorek",
        name: "Thorek Ironbrow",
        factionName: "Ironbrow's Expedition",
      },
    ],
  },
  {
    raceID: "emp",
    name: "Empire",
    type: "order",
    colour: "hsl(352deg 100% 50%)",
    lords: [
      {
        lordID: "karl",
        name: "Karl Franz",
        factionName: "Reikland",
      },
      {
        lordID: "balthasar",
        name: "Balthasar Gelt",
        factionName: "Golden Order",
      },
      {
        lordID: "volkmar",
        name: "Volkmar the Grim",
        factionName: "The Cult of Sigmar",
      },
      {
        lordID: "markus",
        name: "Markus Wulfhart",
        factionName: "The Huntsmarshal's Expedition",
      },
    ],
  },
  {
    raceID: "grn",
    name: "Greenskins",
    type: "chaos",
    colour: "hsl(70deg 91% 39%)",
    lords: [
      {
        lordID: "grimgor",
        name: "Grimgor Ironhide",
        factionName: "Grimgor's Ardboyz",
      },
      {
        lordID: "azhag",
        name: "Azhag the Slaughterer",
        factionName: "Bonerattlaz",
      },
      {
        lordID: "skarsnik",
        name: "Skarsnik",
        factionName: "Crooked Moon",
      },
      {
        lordID: "wurrzag",
        name: "Wurrzag",
        factionName: "The Bloody Handz",
      },
      {
        lordID: "grom",
        name: "Grom the Paunch",
        factionName: "Broken Axe Tribe",
      },
    ],
  },
  {
    raceID: "hef",
    name: "High Elves",
    type: "order",
    colour: "hsl(182deg 64% 50%)",
    lords: [
      {
        lordID: "teclis",
        name: "Teclis",
        factionName: "Order of Loremasters",
      },
      {
        lordID: "tyrion",
        name: "Tyrion",
        factionName: "Lothern",
      },
      {
        lordID: "alarielle",
        name: "Alarielle the Radiant",
        factionName: "Avelorn",
      },
      {
        lordID: "eltharion",
        name: "Eltharion the Grim",
        factionName: "Yvresse",
      },
      {
        lordID: "alith",
        name: "Alith Anar",
        factionName: "Nagarythe",
      },
      {
        lordID: "imrik",
        name: "Imrik",
        factionName: "Knights of Caledor",
      },
    ],
  },

  {
    raceID: "kho",
    name: "Khorne",
    type: "chaos",
    colour: "hsl(352deg 100% 50%)",
    lords: [
      {
        lordID: "skarbrand",
        name: "Skarbrand the Exiled",
        factionName: "Exiles of Khorne",
      },
    ],
  },
  {
    raceID: "klv",
    name: "Kislev",
    type: "order",
    colour: "hsl(182deg 64% 50%)",
    lords: [
      {
        lordID: "katarin",
        name: "Tzarina Katarin",
        factionName: "The Ice Court",
      },
      {
        lordID: "kostaltyn",
        name: "Kostalytn",
        factionName: "The Grand Orthodoxy",
      },
      {
        lordID: "boris",
        name: "Boris Ursus",
        factionName: "Ursun Revivalists",
      },
    ],
  },
  {
    raceID: "lzd",
    name: "Lizardmen",
    type: "order",
    colour: "hsl(15deg 99% 44%)",
    lords: [
      {
        lordID: "mazdamundi",
        name: "Mazdamundi",
        factionName: "Hexoatl",
      },
      {
        lordID: "kroq_gar",
        name: "Kroq-Gar",
        factionName: "Last Defenders",
      },
      {
        lordID: "tehenhauin",
        name: "Tehenhauin",
        factionName: "Cult of Sotek",
      },
      {
        lordID: "tiqtaqto",
        name: "Tiqtaq'to",
        factionName: "Tlaqua",
      },
      {
        lordID: "gor_rok",
        name: "Gor-Rok",
        factionName: "Itza",
      },
      {
        lordID: "nakai",
        name: "Nakai the Wanderer",
        factionName: "Spirit of the Jungle",
      },
      {
        lordID: "oxyotl",
        name: "Oxyotl",
        factionName: "Ghosts of Pahuax",
      },
    ],
  },

  {
    raceID: "nor",
    name: "Norsca",
    type: "chaos",
    colour: "hsl(198deg 32% 56%)",
    lords: [
      {
        lordID: "wulfrik",
        name: "Wulfrik the Wanderer",
        factionName: "World Walkers",
      },
      {
        lordID: "throgg",
        name: "Throgg",
        factionName: "Wintertooth",
      },
    ],
  },

  {
    raceID: "nur",
    name: "Nurgle",
    type: "chaos",
    colour: "hsl(70deg 91% 39%)",
    lords: [
      {
        lordID: "kugath",
        name: "Ku'gath Plaguefather",
        factionName: "Poxmakers of Nurgle",
      },
    ],
  },
  {
    raceID: "ogr",
    name: "Ogre Kingdoms",
    type: "destruction",
    colour: "hsl(15deg 99% 44%)",
    lords: [
      {
        lordID: "skrag",
        name: "Skrag the Slaughterer",
        factionName: "Disciples of the Maw",
      },
      {
        lordID: "greasus",
        name: "Greasus Goldtooth",
        factionName: "Goldtooth",
      },
    ],
  },
  {
    raceID: "skv",
    name: "Skaven",
    type: "destruction",
    colour: "hsl(70deg 91% 39%)",
    lords: [
      {
        lordID: "skrolk",
        name: "Lord Skrolk",
        factionName: "Clan Pestilens",
      },
      {
        lordID: "queek",
        name: "Queek Headtaker",
        factionName: "Clan Mors",
      },
      {
        lordID: "tretch",
        name: "Tretch Craventail",
        factionName: "Clan Rictus",
      },
      {
        lordID: "ikit",
        name: "Ikit Claw",
        factionName: "Clan Skryre",
      },
      {
        lordID: "snikch",
        name: "Deathmaster Snikch",
        factionName: "Clan Eshin",
      },
      {
        lordID: "throt",
        name: "Throt the Unclean",
        factionName: "Clan Moulder",
      },
    ],
  },

  {
    raceID: "sla",
    name: "Slaanesh",
    type: "chaos",
    colour: "hsl(303deg 75% 51%)",
    lords: [
      {
        lordID: "nkari",
        name: "N'Kari",
        factionName: "Seducers of Slaanesh",
      },
    ],
  },
  {
    raceID: "tmb",
    name: "Tomb Kings",
    type: "undead",
    colour: "hsl(43deg 55% 47%)",
    lords: [
      {
        lordID: "settra",
        name: "Settra the Imperishable",
        factionName: "Khemri",
      },
      {
        lordID: "arkhan",
        name: "Arkhan the Black",
        factionName: "Followers of Nagash",
      },
      {
        lordID: "khalida",
        name: "High Queen Khalida",
        factionName: "Court of Lybaras",
      },
      {
        lordID: "khatep",
        name: "Grand Hierophant Khatep",
        factionName: "Exiles of Nehek",
      },
    ],
  },

  {
    raceID: "tzn",
    name: "Tzeentch",
    type: "chaos",
    colour: "hsl(182deg 64% 50%)",
    lords: [
      {
        lordID: "kairos",
        name: "Kairos Fateweaver",
        factionName: "Oracles of Tzeentch",
      },
    ],
  },
  {
    raceID: "vmp",
    name: "Vampire Counts",
    type: "undead",
    colour: "hsl(303deg 75% 51%)",
    lords: [
      {
        lordID: "mannfred",
        name: "Mannfred von Carstein",
        factionName: "Sylvania",
      },
      {
        lordID: "heinrich",
        name: "Heinrich Kemmler",
        factionName: "Barrow Legion",
      },
      {
        lordID: "helman",
        name: "Helman Ghorst",
        factionName: "Caravan of the Blue Roses",
      },
      {
        lordID: "vlad",
        name: "Vlad von Carstein",
        factionName: "Sylvania",
      },
    ],
  },
  {
    raceID: "wef",
    name: "Wood Elves",
    type: "order",
    colour: "hsl(70deg 91% 39%)",
    lords: [
      {
        lordID: "orion",
        name: "Orion",
        factionName: "Talsyn",
      },
      {
        lordID: "durthu",
        name: "Durthu",
        factionName: "Argwylon",
      },
      {
        lordID: "sisters",
        name: "Heralds of Ariel",
        factionName: "Heralds of Ariel",
      },
      {
        lordID: "drycha",
        name: "Drycha",
        factionName: "Wargrove of Woe",
      },
    ],
  },
  {
    raceID: "woc",
    name: "Warriors of Chaos",
    type: "chaos",
    colour: "hsl(15deg 99% 44%)",
    lords: [
      {
        lordID: "kholek",
        name: "Kholek Suneater",
        factionName: "Heralds of the Tempest",
      },
      {
        lordID: "valkia",
        name: "Valkia the Bloody",
        factionName: "Legion of the Gorequeen",
      },
      {
        lordID: "vilitch",
        name: "Vilitch the Curseling",
        factionName: "Puppets of Misrule",
      },
      {
        lordID: "belakor",
        name: "Be'lakor",
        factionName: "Shadow Legion",
      },
      {
        lordID: "sigvald",
        name: "Sigvald the Magnificent",
        factionName: "The Decadent Host",
      },
      {
        lordID: "azazel",
        name: "Azazel",
        factionName: "The Ecstatic Legions",
      },
      {
        lordID: "festus",
        name: "Festus the Leechlord",
        factionName: "The Fecundites",
      },
      {
        lordID: "archaon",
        name: "Archaon the Everchosen",
        factionName: "Warhost of the Apocalypse",
      },
    ],
  },
];

export const CSS_VARS = {
  colour: {
    primary: {
      100: "hsl(190deg 43% 3%)",
      200: "hsl(191deg 34% 7%)",
      400: "hsl(193deg 26% 14%)",
      600: "hsl(195deg 19% 25%)",
      700: "hsl(196deg 11% 56%)",
      900: "hsl(198deg 15% 95%)",
    },

    secondary: {
      500: "hsla(11, 76%, 45%, 1)",
      700: "hsla(8, 43%, 51%, 1)",
      // 700: "hsla(8, 63%, 71%, 1)",
    },

    mono: {
      100: "hsla(190,5%,10%,1)",
      200: "hsla(190,5%,15%,1)",
      300: "hsla(190,5%,25%,1)",
      500: "hsla(190,5%,50%,1)",
      700: "hsla(190,5%,75%,1)",
      900: "hsla(190,5%,95%,1)",
    },

    state: {
      success:
        "linear-gradient(180deg, #7b8c26 0%, #6e7d22 16%, #606e1e 33%, #535e1a 50%, #464f15 66%, #3a4112 83%, #2e340e 100%)",
      caution:
        "linear-gradient(180deg, #f7d65f 0%, #eec73d 16%, #e4ba21 33%, #d6ad1e 50%, #c6a21e 66%, #b7951f 83%, #a88a1f 100%)",
      danger:
        "linear-gradient(180deg, #d44825 0%, #ca4122 16%, #c03b1e 33%, #b5351b 50%, #aa3018 66%, #a02a16 83%, #952513 100%)",
    },
  },

  font: {
    family: {
      heading: `'Cinzel', serif`,
      body: `'Inter', sans-serif`,
    },
    weight: {
      light: 200,
      regular: 400,
      bold: 700,
      extra: 900,
    },
  },

  size: {
    xs: `8px`,
    s: `12px`,
    m: `16px`,
    l: `24px`,
    xl: `48px`,
    xxl: `64px`,
  },

  shadow: {
    strong: `hsl(206 22% 7% / 100%) 0px 10px 38px -10px,
    hsl(206 22% 7% / 40%) 0px 10px 20px -15px`,
  },
};
